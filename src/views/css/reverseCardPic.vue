...
const list = ref<Array<{ name: string, bg: string, show: boolean }>>([])
...
async function animateReverseCard() {
  for (let i = 0; i < list.value.length; i += 1) {
    list.value[i].show = true
    if (i === 0)
      continue
    list.value[i - 1].show = false
...
onMounted(async () => {
  ...
  initConDomRect()
  // 适配响应式
  window.addEventListener('resize', debounce(initConDomRect, 500))
