...
let width = 600
let height = 600
let fireworks: Array<Firework> = []
let g = new p5.Vector(0, 0.4)
...
class Firework {
  ...
  show() {
    ...
    if (!this.isBoom) p.circle(this.pos.x, this.pos.y, this.d)
    for (let i = this.fires.length - 1; i >= 0; i--) {
      this.fires[i].update()
      this.fires[i].show()
      if (this.fires[i].isOver()) {
        this.fires.splice(i, 1)
      if (this.fires.length === 0) this.isAlive = false
      }
    }
  }
  boom() {
    let fireNum = floor(p.random(30, 50))
    for (let i = 0; i < fireNum; i++) {
      let fireVel = p5.Vector.fromAngle(
        p.random(2 * PI),
