...
import { centroid, pointInPolygon, rotateByPoint } from '@/utils'
...
class Shape {
  ...
  updatePoints() {
    ...
    if (choseId === this.id) {
      ...
      const subVec = this.curPos.copy().sub(this.prePos)
...
function mousePressed() {
  ...
  for (const i in shapes) {
    const index = String(8 - Number(i))
    if (shapes.hasOwnProperty(index)) {
      const isIn = pointInPolygon(shapes[index].points, [pressX, pressY])
      if (isIn) {
